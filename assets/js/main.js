...
const initContador = () => {
    ...
    // 28/10/2025 00:00:00 no fuso local
    const start = new Date(2025, 9, 28, 0, 0, 0); // mês 9 = outubro (0-based)
    ...
    const update = () => {
        const now = new Date();
        let diff = now - start;
        if (diff < 0) diff = 0; // Caso acesse antes da data
        ...
        diff %= D;
        const horas = Math.floor(diff / H);
        diff %= H;
        const minutos = Math.floor(diff / MIN);
        diff %= MIN;
        const segundos = Math.floor(diff / MS);

        elDias.textContent = dias;
        elHoras.textContent = pad2(horas);
        elMinutos.textContent = pad2(minutos);
        elSegundos.textContent = pad2(segundos);
    };
